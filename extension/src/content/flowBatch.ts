 ...
 *
 * It could not be right. The header above records a search of real response
 * bodies for any capitalised enum-looking token: there were none. This API
 * does not state a failure in words, so the scan had no true positive
 * available to it.
 *
 * And it could be badly wrong, because the user's own prompt is one of the
 * strings in the record — promptOf pulls it out of the same block. A prompt
 * reading "a city block at night" contains BLOCK; "cancel the wedding"
 * contains CANCEL. The scan ran BEFORE the media check, so a finished video
 * was reported failed, and the prompt was then handed on as the reason:
 * classifyError reads it, finds a word like "prominent" or "violat", and
 * calls it a safety block — which is the one classification that hard-fails
 * with no retry. A working generation, thrown away over its own wording.
 *
 * So failure is not read here at all. The API answers "is it finished"; the
 * page answers "did it fail", and it answers it plainly — a failed generation
 * renders <flow-error-tile> with the reason written in it and a Retry button
 * beside it. getTileState reads that, and the retry pass trusts it.
 */
