...
### Publishing without a shared secret

The token endpoint exists, but the route actually used is an upload form in
the Django admin.

A static token has to be generated by somebody, pasted into Railway, then
pasted again into a shell — three places a long-lived credential gets copied
into a log, a screenshot or a chat window, guarding an endpoint that can
replace the bundle for every user at once. Uploading in the admin needs no new
secret: the session is already authenticated, Django enforces staff status and
CSRF, and nothing sensitive leaves the browser.

`TEMPLATE_PUBLISH_TOKEN` is unset by default, which disables the token
endpoint entirely. Set it only when something automated needs to publish.
