...
const SW_PATH = '../content/flow/sw-bypass.ts';
const IDX_PATH = '../content/flow/index.ts';
...
describe('deciding whether it finished', () => {
  ...
  it('does not read a failure out of the record text', () => {
    /* This used to scan every string for FAIL/SAFETY/BLOCK/REJECT/CANCEL and
       report the first hit as the failure reason. It was removed: a search of
       real response bodies for any capitalised enum-looking token found none,
       so the scan had no true positive available — while the user's prompt
       sits in the same record and could trip it at any time. */
  ...
  it('does not fail a finished video over a word in its own prompt', () => {
    /* The bug this cost, next door: "block", "cancel" and "fail" are ordinary
       English. The scan ran before the media check, so a completed generation
       was reported failed and the prompt was passed on as the reason — where
       classifyError reads "prominent" and calls it a safety block, the one
       class that never retries. */
    for (const prompt of [
...
describe('the source itself', () => {
  it('carries no control character in any pattern', () => {
    ...
    const src = fs.readFileSync(path.resolve(__dirname, '../content/flow/flowBatch.ts'), 'utf8');
...
describe('saying the interception is alive', () => {
  ...
  it('carries a build marker readable from the page', () => {
    ...
    expect(SW).toMatch(/__af_interceptor_build = 'batchexecute-2'/);
  });
...
describe('the active check', () => {
  ...
  it('only keeps a request that actually reported statuses', () => {
    ...
    expect(SW2).toMatch(/found > 0 && !lastBatchIntroducedNewId && typeof reqBody === 'string'/);
  });

  it('never keeps the request that SUBMITTED a generation', () => {
    /* The template is replayed verbatim, with credentials, whenever
       observation has to be recovered. If the captured request were the
       submit, recovery would generate a second video and charge for it —
       from a path whose entire purpose is to look rather than to act.

       A submit is the only request that introduces a media id nobody has
       seen; a status poll re-describes known ones. On batchexecute the rpc
       name is an opaque code and this fork has no pending-prompt check, so
       that is the only reliable discriminator available. */
    expect(SW2).toMatch(/lastBatchIntroducedNewId = statuses\.some\(/);
    expect(SW2).toMatch(/!seenMediaIds\.has\(/);
    expect(SW2).toMatch(/let lastBatchIntroducedNewId = true/);
  });
  ...
  it('reports success only when the reply described a generation', () => {
    expect(SW2).toMatch(/relayBatch\(req\.url, await res\.text\(\)\) > 0/);
  });
